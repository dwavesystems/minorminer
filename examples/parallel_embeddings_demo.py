...
def main():
    ...
    smallest_tile = {"chimera": 1, "pegasus": 2, "zephyr": 1}  # Minimum tile sizes for each topology
    generators = {
    ...
    for source_topology in topologies:
        ...
        print(f"Evaluating embeddings for source topology '{source_topology}' with sublattice size {sublattice_size_S}.")
    ...
    for topology in topologies:
        ...
        # Validate embeddings
        assert all(set(emb.keys()) == set(S.nodes()) for emb in embs), "Mismatch in source nodes."
        assert all(set(emb.values()).issubset(set(T.nodes())) for emb in embs), "Mismatch in target nodes."
        value_list = [v for emb in embs for v in emb.values()]
        assert len(set(value_list)) == len(value_list), "Duplicate target nodes in embeddings."
        ...
        assert all(set(emb.keys()) == set(S.nodes()) for emb in embs_direct), "Mismatch in source nodes."
        assert all(set(emb.values()).issubset(set(T.nodes())) for emb in embs_direct), "Mismatch in target nodes."
        value_list_direct = [v for emb in embs_direct for v in emb.values()]
        assert len(set(value_list_direct)) == len(value_list_direct), "Duplicate target nodes in direct embeddings."
