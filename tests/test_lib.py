...
def check_args(prob, hard, initial_chains=None, fixed_chains=None, restrict_chains=None, skip_initialization=False):
    ...
    if fixed_chains is not None:
        for v,chain in fixed_chains.iteritems():
    ...
    if initial_chains is not None:
        for v,chain in initial_chains.iteritems():
        ...
        if skip_initialization:
            for u,v in probg.edges():
                edgelord = {z for q in initial_chains[v] for z in hardg.neighbors(q)}|set(initial_chains[v])
                assert set(initial_chains[u])&edgelord, "%s and %s are connected as variables but not as initials"%(u,v)

    if restrict_chains is not None:
        fullset = set(hardg.nodes())
        for v,chain in restrict_chains.iteritems():
        ...
        for u,v in probg.edges():
            edgelord = {z for q in restrict_chains.get(v,fullset) for z in hardg.neighbors(q)}|set(restrict_chains.get(v,fullset))
            assert set(restrict_chains.get(u,fullset))&edgelord, "%s and %s are connected as variables but not as domains"%(u,v)
...
def test_fail_timeout(n,t):
    ...
    Cn = Chimera(n) #into this here chimera, and it might work but we'll time out
