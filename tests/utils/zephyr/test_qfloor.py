...
from minorminer.utils.zephyr.qfloor import QuoFloor, QuoTile
...
class TestQuoFloor(unittest.TestCase):
    ...
    def test_ver_zns(self) -> None:
        ...
        nodes += [
            n + p for n in nodes for p in [PlaneShift(2, 0), PlaneShift(0, 2), PlaneShift(2, 2)]
        ]
    ...
    def test_hor_zns(self) -> None:
        ...
        for m in [4, 5]:
            ...
            nodes2 += [
                n + p
                for n in nodes2
                for p in [PlaneShift(2, 0), PlaneShift(0, 2), PlaneShift(2, 2)]
            ]
