...
class ZSurvey:
    ...
    def get_shape_coord(G: nx.Graph | DWaveSampler) -> dict[str, ZShape | str]:
        ...
        def graph_shape_coord(G: nx.Graph) -> dict[str, int | str]:
            ...
            if G_top != "zephyr":
                raise ValueError(
                    f"Expected a graph with zephyr topology, got {G_top}"
                    )
        ...
        def sampler_shape_coord(sampler: DWaveSampler) -> dict[str, int | str]:
            ...
            if sampler_top.get("type") != "zephyr":
                raise ValueError(
                    f"Expected a sampler with zephyr topology, got {sampler_top}"
                    )
            ...
            for e in edges:
                ...
                if len(e) != 2:
                    raise ValueError(
                        f"Expected tuple of length 2 in edgelist, got {e}"
                        )
        ...
        if isinstance(G, DWaveSampler):
            ...
            raise TypeError(
                f"Expected G to be networkx.Graph or DWaveSampler, got {type(G)}"
                )
    ...
    def missing_edges(self) -> set[ZEdge]:
        ...
        parent_edges = [
            ZEdge(
                ZNode(coord=u, shape=self.shape),
                ZNode(coord=v, shape=self.shape)
                )
            for (u, v) in dnx.zephyr_graph(m=self._shape.m, t=self._shape.t, coordinates=True).edges()
            ]
    ...
    def extra_missing_edges(self) -> set[ZEdge]:
        ...
        return {
                ...
                if e[0] in self._nodes
                and e[1] in self._nodes
            }
    ...
    def neighbors(
        ...
        if not isinstance(v, ZNode):
            raise TypeError(
                f"Expected v to be ZNode, got {v}"
                )
        if v in self.missing_nodes:
            return {}
        return {
            v_nbr
            for v_nbr in v.neighbors(nbr_kind=nbr_kind)
            if ZEdge(v, v_nbr) in self._edges
            }
    ...
    def calculate_external_paths_stretch(self) -> dict[UWKJ, set[ZSE]]:
        ...
        return {
            uwkj: self._ext_path(uwkj)
            for uwkj in uwkj_vals
            }
