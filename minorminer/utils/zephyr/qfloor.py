...
from minorminer.utils.zephyr.node_edge import ZNode, ZEdge, ZShape
...
class QuoTile:
    ...
    def zns(self, new_zns: Iterable[ZNode]) -> None:
        ...
        if not isinstance(new_zns, Iterable):
            raise TypeError(
                f"Expected {new_zns} to be Iterable[ZNode], got {type(new_zns)}"
                )
        for zn in new_zns:
            if not isinstance(zn, ZNode):
                raise TypeError(
                    f"Expected elements of {new_zns} to be ZNode, got {type(zn)}"
                    )
            if not zn.is_quo():
    ...
    def edges(
        ...
        _edges = {
            edge
            for zn in zns
            for edge in zn.incident_edges(nbr_kind=nbr_kind, where=where_tile)
            }
        return list(_edges)
...
class QuoFloor:
    ...
    def tile_connector(
        self,
        new_connector: dict[tuple[int], PlaneShift]
        ) -> None:
    ...
    def check_dim_corner_qtile_compatibility(
        ...
        try:
            ...
            raise ValueError(
                f"{dim, corner_qtile} are not compatible"
                )
    ...
    def dim(self, new_dim: Dim | tuple[int]) -> None:
        ...
        if not isinstance(new_dim, Dim):
            raise TypeError(
                f"Expected dim to be Dim or tuple[int], got {type(new_dim)}"
                )
        if not all(isinstance(x, int) for x in new_dim):
            raise TypeError(
                f"Expected Dim elements to be int, got {new_dim}"
                )
        if any(x <= 0 for x in new_dim):
            raise ValueError(
                f"Expected elements of Dim to be positive integers, got {Dim}"
                )
        if hasattr(self, "_corner_qtile"):
            self.check_dim_corner_qtile_compatibility(
                dim=new_dim, corner_qtile=self._corner_qtile)
        self._dim = new_dim
    ...
    def corner_qtile(self, new_qtile: QuoTile) -> None:
        ...
        if not isinstance(new_qtile, QuoTile):
            raise TypeError(
                f"Expected corner_qtile to be QuoTile, got {type(new_qtile)}"
                )
        ...
        for x, y, *_ in ccoords:
            x_values[x].append(y)
            y_values[y].append(x)
        x_diff = {x: max(x_list) - min(x_list)
                    for x, x_list in x_values.items()}
        y_diff = {y: max(y_list) - min(y_list )
                    for y, y_list in y_values.items()}
        ...
        for y, xdiff in y_diff.items():
            if abs(xdiff) >= y_jump:
                raise ValueError(
                    f"This tile may overlap other tiles on {y = }"
                    )
        for x, ydiff in x_diff.items():
            if abs(ydiff) >= x_jump:
                raise ValueError(
                    f"This tile may overlap other tiles on {x = }"
                    )
        if hasattr(self, "_dim"):
            self.check_dim_corner_qtile_compatibility(
                dim=self._dim, corner_qtile=new_qtile)
    ...
    def qtiles(self) -> dict[tuple[int], QuoTile]:
        ...
        """
        if any(par is None
               for par in (self._dim, self._corner_qtile, self._tile_connector)
               ):
    ...
    def zns(self) -> dict[tuple[int], list[ZNode]]:
        ...
        return {
            xy: xy_tile.zns
            for xy, xy_tile in self.qtiles.items()
            }
