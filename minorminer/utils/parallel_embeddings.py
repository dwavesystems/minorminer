...
def shuffle_graph(
    ...
    Args:
        ...
        seed: When provided, is used to shuffle the order of nodes and edges in
            the source and target graph. This can allow sampling from otherwise
...
def lattice_size(T: nx.Graph) -> int:
    """Determines the cellular (square) dimension of a ``dwave.graphs`` lattice
    ...
    Args:
        T:
            The target graph in which to embed. The graph must be of type
            Zephyr, Pegasus or Chimera and constructed by one of the graph
            generators in ``dwave.graphs.topologies``.
...
def _local_graph(T=None, T_family=None, T_kwargs=None):
    if T_family is None:
        if T is None:
            raise ValueError(
                "Either the target graph T, or its lattice specification "
                "T_family and T_kwargs, must be provided"
            )
        return T.copy()
    else:
        if T_kwargs is None:
        ...
        if T_family == "chimera":
            return dwave.graphs.chimera_graph(**T_kwargs)
        elif T_family == "pegasus":
            return dwave.graphs.pegasus_graph(**T_kwargs)
        elif T_family == "zephyr":
            return dwave.graphs.zephyr_graph(**T_kwargs)
        else:
            raise ValueError("Unsupported graph topology")
...
def find_sublattice_embeddings(
    ...
    *,
    ...
    Selects a sublattice (subgrid) of the graph T using ``dwave.graphs``
    ...
    Args:
        ...
        T: The target graph in which to embed. Either the graph should be of type
            Chimera, Pegasus or Zephyr and generated by ``dwave-graphs``, or the
        ...
        tile: A graph constructed by ``dwave-graphs`` applied as a displaceable mask
        ...
        sublattice_size: Parameterizes the tile when it is not provided
           ...
           of a square sublattice (parameter m of the ``dwave-graphs`` graph
        ...
        T_family: 'chimera', 'pegasus' or 'zephyr' arguments result in the target graph
            being cast to the corresponding ``dwave-graphs`` graph family.
    ...
    if T is not None and T.graph.get("family", T_family) not in {
        ...
        raise ValueError(
            "If T is not a 'dwave-graphs' graph, T_family must be specified"
        )
    ...
    if family == "chimera":
        sublattice_mappings = dwave.graphs.chimera_sublattice_mappings
        t = _T.graph["tile"]
        if tile is None:
            tile = dwave.graphs.chimera_graph(m=sublattice_size, n=sublattice_size, t=t)
        elif (
            use_filter
        ...
        sublattice_mappings = dwave.graphs.pegasus_sublattice_mappings
        if tile is None:
            tile = dwave.graphs.pegasus_graph(m=sublattice_size)
        elif (
            use_filter
            and embedding_feasibility_filter(S, tile, not one_to_iterable) is False
        ):
        ...
        sublattice_mappings = dwave.graphs.zephyr_sublattice_mappings
        t = _T.graph["tile"]
        if tile is None:
            tile = dwave.graphs.zephyr_graph(m=sublattice_size, t=t)
        elif (
            use_filter
            and embedding_feasibility_filter(S, tile, not one_to_iterable) is False
        ):
        ...
        raise ValueError(
            "source graph must be a graph constructed by one of the "
            "dwave.graphs.topologies.{chimera,pegasus,zephyr} generators"
        )
