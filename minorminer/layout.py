...
def get_embedding(S, T, layout="kamada_kawai_2d", placement="closest", chains="singleton", **kwargs):
    """
    Given a placement (a map from vertices of S to vertices of T), 
    form chains that will serve as embedding hints for minerminor.
...
def get_placement(S_layout, T_layout, strategy="closest"):
    ...
    strategy : string (default "closest")
        Different placement strategies to use. Implemented strategies are:
            - "closest" : For each vertex u in S_layout and each vertex v in T_layout, 
                            map u to the v with minimum Euclidean distance (||u - v||_2).
            - "injective" : This is the same as the "closest" strategy, except the
                            map is injective. This runs in O(|S||T|log(|T|)) time.
...
def get_chains(S, T, placement, strategy="singleton"):
    ...
    strategy : string (default "singleton")
        Different chain creation strategies to use. For each of the following descriptions
        we assume that vertex u of S is mapped to vertex v of T. Implemented strategies are:
            - "singleton" : A chain is of the form [v] for u.
            - "extend" : Heuristically (with minorminer) compute optimal paths between v and 
                            the image of each neighbor of u. The chain for u is the union of
                            v together with these open paths.
            - "neighborhood" : A chain is of the form N_T(v) (closed neighborhood of v in T) 
                                for u.
            - "neighborhood_extension" : First the extend algorithm is run to get extended chains,
                                            then the closed neighborhood of that chain is returned.

    Returns
    -------
    chains: dict
        A mapping from vertices of H (keys) to chains of G (values).
    """
...
def closed_neighbors(H, u, second=False):
    neighbors = set(v for v in H.neighbors(u))
    if second:
        return set((u,)) | neighbors | set(w for v in neighbors for w in H.neighbors(v))
