...
class TestLayout(unittest.TestCase):
    ...
    def test_chimera(self):
        """
        Tests that the layout is correct for Chimera(4); i.e. that it matches the pre-computed values in [-1, 1]^2.
        """
        G = dnx.chimera_graph(4)
        my_chimera = layout.chimera(G)

        for v, p in pre_computed_chimera.items():
            for i, x in enumerate(p):
                coordinate = my_chimera[v][i]
                self.assertAlmostEqual(coordinate, x)
                self.assertGreaterEqual(coordinate, -1)
                self.assertLessEqual(coordinate, 1)
